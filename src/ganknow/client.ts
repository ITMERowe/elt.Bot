...
async function fetchRenderedFeed(creatorUrl: string): Promise<Post[]> {
  ...
  try {
    ...
    const posts = await page.$$eval('.post-card', cards => cards.map(card => {
      ...
      const normalize = (value: string) => value.replace(/\s*-\s*karamelt\s*$/i, '').replace(/\s+/g, ' ').trim().toLowerCase();
