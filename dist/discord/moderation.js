...
exports.handleForbiddenChannelMessage = handleForbiddenChannelMessage;
const config_1 = require("../config");
const activeBans = new Set();
async function deleteMemberMessagesFromGuild(guild, authorId, authorTag, maxAgeMs) {
        ...
        try {
            ...
            while (true) {
                ...
                for (const candidate of batch.values()) {
                    if (candidate.author.id !== authorId)
                        continue;
                    if (cutoff !== undefined && candidate.createdTimestamp < cutoff)
                        continue;
                    try {
...
async function handleForbiddenChannelMessage(message) {
    if (!config_1.config.forbiddenChannelIds.includes(message.channelId) || message.author.bot)
        return;
