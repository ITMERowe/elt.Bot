...
async function main() {
    ...
    try {
        console.info('[INFO] Running initial fetch for', config_1.config.ganknowCreatorUrl);
        ...
        await (0, storage_1.writeState)({ lastPostId: null, lastSentDisc: null, seenPosts: [] });
        console.info('[INFO] Cleared existing data/state.json');
        const stopRecentSpinner = (0, spinner_1.startSpinner)('Fetching recent posts from creator feed');
        const posts = await (0, client_1.fetchAllPostsFromCreator)(config_1.config.ganknowCreatorUrl);
        ...
        if (enriched.length > 0) {
            ...
            const stopWriteSpinner = (0, spinner_1.startSpinner)('Writing state to data/state.json');
            await (0, storage_1.markSeenPostsByPosts)(enriched);
